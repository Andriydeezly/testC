...
public:
	Point() {
		x= 0;
		y= 0;
		cout << "Конструктор визвався\t " << this << endl;
...
int  main() {
	...
	*/
	...
	cout << "Перевернута  "<< str <<"\n";*/

	/* char *dictionary[][2] = {
"карандаш","Инструмент для",
"клавиатура","Устройство ввода",
"ружйо","Плечевое огнестрельноэ",
"Самолет","Воздушноэ судно",
"сеть","Група между",
"","" };
char word[80];
int i;
...
char *get_substr(char *sub, char *str) {
	...
	/*char str1[80] = "ето проверка";
char str2[80] = "1234563";
strcpy(str1, "ето проверка");
...
*s1 = '\0';
	...
	/*int num;
do {
	cout << "ввиводимиэ числа";
		cin >> num;
		if (num != -1)
			cout<< "среднэ равно  "<<running_avg(num)<<"\n";
} while (num > -1);
return 0;
}
...
char key = 88;
cout << "Ввод нового сообщения " << "\n";
for (int i = 0; i < strlen(msg); i++)
	msg[i] = msg[i] ^ key;

cout << "Зашифрований текст  " << msg << "\n";
for (int i = 0; i < strlen(msg); i++)
	msg[i] = msg[i] ^ key;
cout << "Розшифроване повыдомлення   " << msg << "\n";
return 0;*/

	/*char ch = 'T';
cout << "Исходнаэ значения в доъ коде:  \n";
for (int i = 0; i < 8; i++){
	ch = rrotate(ch, 1);
	binary(ch);}
return 0;
...
void binary(unsigned int u) {
	...
	cout << "используэм bigQ для хранения латинского алфавита. \n";
	for (i = 0; i < 26; i++) 
	    bigQ.put('A' + i);
	
	cout << "виведем на екран елементи bigQ. \n";
	for (i = 0; i < 26; i++) {
		ch =bigQ.get();
		if (ch != 0) cout << ch;}
	cout << "\n\n";

	cout << "используэм smallQ для розпызнання ошибок. \n";
		for (i = 0; i < 5; i++) {
			cout << "питаэмся записать" << 
				(char)('Z' - i);
			smallQ.put('Z' - i);
			cout << "\n";
		}
